...
contract CloversControllerV3 is Ownable {
    ...
    modifier onlyOwnerOrOracle () {
        ...
        require(msg.sender == _owner || msg.sender == oracle);
        _;
    }
    ...
    */
    function getMovesHash(uint _tokenId) public view returns (bytes32) {
        return keccak256(abi.encodePacked(IClovers(clovers).getCloverMoves(_tokenId)));
    }
    /**
    ...
    */
    function calculateReward(uint256 _symmetries) public view returns (uint256) {
...
/*
    ...
    function getPrice(uint256 _symmetries) public view returns(uint256) {
    ...
    */
    function claimClover(bytes28[2] memory moves, uint256 _tokenId, uint256 _symmetries, bool _keep) public payable notPaused returns (bool) {
        ...
        require(getCommit(movesHash) == address(0));

        setCommit(movesHash, msg.sender);
        if (stakeWithGas > 0) {
            setStake(movesHash, stakeWithGas);
            address payable _clovers = address(uint160(clovers));
            _clovers.transfer(stakeWithGas);
        }
        ...
        if (_keep && price > 0) {
            ...
            if (IClubToken(clubToken).balanceOf(msg.sender) < price) {
                IClubTokenController(clubTokenController).buy.value(msg.value.sub(stakeWithGas))(msg.sender);
            }
    ...
    */
    function retrieveStake(uint256 _tokenId) public payable returns (bool) {
        ...
        require(isVerified(_tokenId) || msg.sender == _owner || msg.sender == oracle);

        uint256 stake = getStake(movesHash);
        addSymmetries(_tokenId);
        address committer = getCommit(movesHash);
        require(committer == msg.sender || msg.sender == _owner || msg.sender == oracle);
